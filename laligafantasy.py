...
CURRENT_WEEK = 17  # la semana actual de la liga
...
def get_player_last_5_weeks(name):
    """
    Devuelve stats de un jugador:
      - nombre
      - puntos totales
      - media últimos 5 semanas (rellenando con 0 si no jugó)
    """
    name_lower = name.lower()
    players = fetch_json(PLAYERS_URL)
    
    for player in players:
        if player.get('nickname', '').lower() == name_lower:
            total_points = player.get('points', 0)
            week_points_dict = {wp['weekNumber']: wp['points'] for wp in player.get('weekPoints', [])}
            
            last_5_points = []
            for w in range(CURRENT_WEEK, CURRENT_WEEK-5, -1):
                last_5_points.append(week_points_dict.get(w, 0))
            
            last_5_average = sum(last_5_points) / 5
